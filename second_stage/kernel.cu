...
#define TILE 32
__device__ int temp[8192 * (8192 / TILE)];
__global__ void firstStage(int* changes, int* account, int clients, int periods) {
    __shared__ volatile int scan[TILE * TILE];
    int index =
        blockIdx.x * blockDim.x + threadIdx.x + clients * (blockIdx.y * blockDim.y + threadIdx.y);
    scan[threadIdx.x + threadIdx.y * TILE] = changes[index];
}
...
void solveGPU(int* changes, int* account, int* sum, int clients, int periods) {
    dim3 grid(clients / TILE, periods / TILE);
    dim3 block(TILE, TILE);
    firstStage<<<grid, block>>>(changes, account, clients, periods);
